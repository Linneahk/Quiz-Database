...
function subscribeToSession() {
  ...
  // Listen for own answers being added
  channel.on('postgres_changes', {
    event: 'INSERT', schema: 'public', table: 'session_answers',
    filter: `player_id=eq.${playerId}`
  }, () => {
    _newAnswerSinceLastRefresh = true
    refreshAnswers()
  })
  channel.subscribe()
}

// Refresh answers berre viss eleven har svara på noko sidan sist
let _newAnswerSinceLastRefresh = false
document.addEventListener('visibilitychange', () => {
  if (!document.hidden && playerId && _newAnswerSinceLastRefresh) {
    _newAnswerSinceLastRefresh = false
    refreshAnswers()
  }
})
...
async function endHunt() {
  ...
  const { data: player } = await sb.from('session_players').select('total_score, hunt_started_at, hunt_finished_at').eq('id', playerId).single()
  ...
  const elapsed = huntStartTime ? Math.floor((Date.now() - huntStartTime) / 1000) : 0
...
window.confirmLeave = async function () {
  ...
  show('pin-screen')
  if (pid) {
    localStorage.setItem('quiz_left_' + sid, JSON.stringify({
      playerId: pid,
