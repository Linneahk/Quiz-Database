...
async function restoreSession(identity) {
  sessionId = identity.sessionId; quizId = identity.quizId; playerId = identity.playerId; playerToken = identity.token
...
window.joinGame = async function () {
  ...
  // Per-spelar hemmelegheit — krevst av serveren for å svare/oppdatere (hindrar juks og sabotasje)
  const token = crypto.randomUUID() + '-' + crypto.randomUUID()
  const { data: p, error } = await sb.from('session_players').insert({ session_id: sessionId, nickname: nick, avatar_color: color, player_token: token }).select('id').single()
  if (error) {
...
async function refreshAnswers() {
  // Eigne svar hentast via token-verifisert RPC (anon kan ikkje lese session_answers direkte)
